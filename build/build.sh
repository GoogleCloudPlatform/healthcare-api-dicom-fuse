...
if [[ "${cp_to_dicom_store_result}" != 0 ]]; then
  echo "Copying to DICOM Store failed!"
  exit 1
fi
if [[ "${cp_from_dicom_store_result}" != 0 ]]; then
  echo "Copying from DICOM Store failed!"
  exit 1
fi
...
if [[ "${rm_result}" != 0 ]]; then
  echo "Removing 111.dcm instance in ${dicom_store_name} DICOM Store failed!"
  exit 1
fi